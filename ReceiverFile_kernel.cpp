/*
   ...
   Real Time Linux Kernel Module Code for Receiver End   
   Description:  
	1-Receives the data from the Serial Port.
	2-Puts each Package of data in the FIFO named Serial_FIFO_NUM
	3-When 5 Packages are put in the FIFO, user space application is notified
...
#include "common.h"	/* defitions for FIFO */
...
MODULE_LICENSE("GPL");
#endif
static RT_TASK serial_receiver_task;	/* we'll fill this in with our task */
static RTIME serial_receiver_period_ns = 50000000;   /* nanoseconds, 20Hz*/	/* timer period, in nanoseconds */
#define BAUD 115200 
#define NUMBITS 8
#define STOPBITS 1
void serial_receiver_function(int arg)
{
  unsigned int data_size = 367;
  unsigned char data_byte[data_size];  /* header : 4 byte, 181 data : 362 bytes, Tail: 1 byte*/
  unsigned char status[2]={'O','K'};   
  int total_pkg_num=5;
printk("Receive data\n");
  int count = 0;
  int i = 0;
  ...
  while(1){
    ...
    rt_spread_timed(COM1, (void *)data_byte, data_size, DELAY_FOREVER);
    rt_spclear_rx(COM1);
    /*TODO : need to confirm the correctness of data */
    if(data_byte[0] == 'A' && data_byte[1] == 'S' && data_byte[2] == 'C' && data_byte[3] == 'I' && data_byte[data_size-1] == 0x01)
    {
        /*Write data into FIFO*/
	rtf_put(Serial_FIFO_NUM, (void *)data_byte, data_size);
	printk(" Standard Data Package is received.\n");
	    count++;
     }
	/* Inter Process Communication */   
 	if (count==total_pkg_num)
	{
	/* Notify the User_app about the reception of 5 Packages */
	rtf_put(Status_FIFO_NUM, &status, sizeof(status));   
	count=0;
	}
    /* Wait one period */
 rt_task_wait_period();
...
{
  RTIME serial_receiver_period_count;	/* requested timer period, in counts */
...
RT_SP_NO_HAND_SHAKE, RT_SP_FIFO_DISABLE);
  rt_spclear_rx(COM1);
  /* Set up FIFO */
  retval = rtf_create(Serial_FIFO_NUM, RTF_SIZE);
  if (retval) {
    printk("could not create RT-FIFO %d\n", Serial_FIFO_NUM);
    return retval;
  }
  rtf_reset(Serial_FIFO_NUM);	/* clear it out */
/*Status FIFO Setup */
retval = rtf_create(Status_FIFO_NUM, RTF_SIZE);   /* A FIFO created for sending Status signal to User space application */
  if (retval) {
    printk("could not create RT-FIFO %d\n", Status_FIFO_NUM);
    return retval;
  }
  rtf_reset(Status_FIFO_NUM);
  /*
  ...
  */
  ...
  /*
    ...
    messages.
  */
  serial_receiver_period_count = nano2count(serial_receiver_period_ns);
  timer_period_count = start_rt_timer(serial_receiver_period_count);
  printk("periodic_task: requested %d counts, got %d counts\n",
	    (int) serial_receiver_period_count, (int) timer_period_count);
  /*
  ...
  */
  retval = 
    rt_task_init(&serial_receiver_task,	/* pointer to our task structure */
		 serial_receiver_function, /* the actual timer function */
   ...
   */
  retval = 
    rt_task_make_periodic(&serial_receiver_task, /* pointer to our task structure */
			  /* start one cycle from now */
			  rt_get_time() + serial_receiver_period_count, 
			  serial_receiver_period_count); /* recurring period */
  if (0 != retval) {
    if (-EINVAL == retval) {
      /* task structure is already in use */
...
{
  int retval;
  retval = rt_task_delete(&serial_receiver_task);
  if (0 !=  retval) {
    if (-EINVAL == retval) {
